...
#  cur.execute("SELECT date,open,high,low,close from stocks where ticker='"+tag+"' order by date desc limit "+str(limit))
  cur.execute("SELECT date,prod_id,asset_id,sum_inv,asset_price from assets where prod_id="+tag+" order by date desc limit "+str(limit))
...
# shift 10 days 
     ...
     _last_20_days[19] = (float)(_matrix[row][4])
#     print _matrix[row], _last_20_days[19]
     if (limit-row)>19:
        _avg20 = 0.0
...
#        for j in range(0,20): _last_20_deviation[j]=(float)(_last_20_days[j]-_avg20)
        for j in range(0,20): 
           _last_20_deviation[j]=math.pow((float)(_last_20_days[j]-_avg20),2)
