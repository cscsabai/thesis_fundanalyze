...
if __name__ == '__main__':
  ...
  if (len(_product) == 0): 
    print("empty response from db. Existing product???")
    exit(0)
  product = _product[0]
#  print product
...
#          print ["GOLDEN:","DEATH:"][_rawcrosses[len(_rawcrosses)-1][1] == 2],_matrix[row][0],_rsi_lane,_matrix[row][4]
          if (_matrix[row][4] >=30 and _matrix[row-1][4] < 30 and _inside!=1):
            if (len(_rawcrosses)-1 >4 and _rawcrosses[len(_rawcrosses)-1][1] == 2):
              _avgxes = ((_rawcrosses[len(_rawcrosses)-2][0]-_rawcrosses[len(_rawcrosses)-3][0]) + (_rawcrosses[len(_rawcrosses)-1][0]-_rawcrosses[len(_rawcrosses)-2][0]) + (_rawcrosses[len(_rawcrosses)-3][0]-_rawcrosses[len(_rawcrosses)-4][0])) /3
              _currxes = _matrix[row][0]-_rawcrosses[len(_rawcrosses)-1][0]
#              print "DEATH with rsi>30:",_matrix[row][0]-_rawcrosses[len(_rawcrosses)-1][0],_rawcrosses[len(_rawcrosses)-1][0]-_rawcrosses[len(_rawcrosses)-2][0]
#              print "+++:",_rawcrosses[len(_rawcrosses)-2][0]-_rawcrosses[len(_rawcrosses)-3][0],_avgxes
              if (_currxes.days > _avgxes.days *0.6):
                print "BESZALLO... (T): ",_matrix[row][0],_matrix[row][1]
                _units = _money / _matrix[row][1]
                _skip = 4
                _inside = 1
                _last_price = _matrix[row][1]
                _last_date = _matrix[row][0]
                _last_days = row
          if (_matrix[row][4] <=50 and _matrix[row-1][4] > 50 and _inside == 1 and _skip ==0 and _rsi_lane == 2):
            print "KISZALLO: ",_matrix[row][0],_matrix[row][1], row-_last_days
            print "%s %s %.03f%% %d" % (_last_date, _matrix[row][0], ((_matrix[row][1]/_last_price-1)*100), row-_last_days)
            _money = _units * _matrix[row][1]
            _lmoney = _money
            _inside = 0

        if (_matrix[row][4] <= 50): _rsi_lane=1
        if (_matrix[row][4] <= 30): _rsi_lane=0
        if (_matrix[row][4] >= 70): _rsi_lane=2

  if (_inside == 1):
    print "!Left inside: ",_last_price,len(_matrix)-1-_last_days
    if (_matrix[len(_matrix)-1][1] >= _last_price):
       _money =_units * _matrix[len(_matrix)-1][1]  
    else:
       _money = _lmoney
  print "Natural increment[%s]: (%.06f -> %.06f) %.03f%% %d"%(_matrix[0][0],_matrix[0][1],_matrix[len(_matrix)-1][1],(_matrix[len(_matrix)-1][1]/_matrix[0][1]-1.0)*100,len(_matrix)-1)
